    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.status == 1) {
                ...
                $('.product-summary .row').html(response.html);


                $('.count').html(response.count_items);

                $('.cart-total .price').text(response.total_format + ' VNĐ');
        ...
        error: function (response) {
            bootbox.alert('Có lỗi xảy ra vui lòng thử lại sau');
        }
    });
}
